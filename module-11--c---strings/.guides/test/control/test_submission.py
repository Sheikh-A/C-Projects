...
sub_path = '{}/submit'.format(ws_path)
...
def check_for_required_files(asg_overview):
    ...
    for (_, _, filenames) in os.walk('{}'.format(sub_path)):
        ...
        break
    print("\n >\tVerifying presence of required submission files in submit/ folder")
    missing = 0
...
def transfer_misplaced_files(asg_overview):
    """ Run through the workspace to see if there are any files misplaced (student put them
    in the workspace rather than submit file). Move them, if so
    """
    required_files = asg_overview['required_files']
    ws_files, sub_files = [], []
    for (_, _, filenames) in os.walk('{}'.format(ws_path)):
        ws_files.extend(filenames)
        break
    for (_, _, filenames) in os.walk('{}'.format(sub_path)):
        sub_files.extend(filenames)
        break
    for req_file in required_files:
        print(" >\t\t - Searching for {} ".format(req_file))
        if req_file not in sub_files:
            print(" >\t\t\t\033[91mNot in submit folder\033[0m")
            if req_file not in ws_files:
                print(" >\t\t\t\033[91mNot in workspace either\033[0m")
            else:
                print(" >\t\t\tFound in workspace, copying into submit folder")
                os.system('cp {}/{} {}/'.format(ws_path, req_file, sub_path))
        else:
            print(" >\t\t\tFound in submit folder")
...
def main():
    ...
    try:
        ...
        print("")
        print(" >\tChecking for misplaced files (those in workspace, not submit folder)")
        transfer_misplaced_files(asg_overview)
